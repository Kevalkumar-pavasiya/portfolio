...
$answer=$_POST['answer'];
$name=$_POST['name'];
$sql="update  aloud set name=:name, question=:question, answer=:answer where id=:id";
$query = $dbh->prepare($sql);
$query->bindParam(':name',$question,PDO::PARAM_STR);
...
header('location:manage-read-aolud.php');
...
<head>
        ...
        <meta charset="utf-8" />
        <title>Edit read aloud | The Vision Overseas</title>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <form role="form" method="post">
                                            ...
                                            <?php 
$id=intval($_GET['id']);
$sql = "SELECT * from  speakig where id=:id";
$query = $dbh -> prepare($sql);
$query->bindParam(':id',$id,PDO::PARAM_STR);
$query->execute();
...
<?php }}?>
                                            <div class="form-group">
                                                <label for="formrow-firstname-input">Upadate_Name</label>
                                                 <textarea type="text" id="textarea" class="form-control"  value="<?php echo htmlentities($result->name);?>" name="name" required="required"  rows="8" ></textarea>    
                                               
                                            </div>

                                            <div class="form-group">
                                                <label for="formrow-firstname-input">Upadate_Paragraph</label>
                                                 <textarea type="text" id="textarea" class="form-control"  value="<?php echo htmlentities($result->question);?>" name="question" required="required"  rows="8" ></textarea>    
